...
app.get("/transformed/get-presigned-url", async (req, res) => {
  ...
  if (!query.type || !query.fileName || !query.mime) {
    return res.status(400).json({
      msg: "Query not found",
    });
  }
...
app.get("/image/transformed/get-presigned-url", async (req, res) => {
  const query = req.query;
  ...
  if (query.format === "auto" || query.format === undefined) {
    query.format = "jpeg";
  }
